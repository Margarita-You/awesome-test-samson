...
export const ToggleSwitch: React.FC<Props> = ({ defaultEnabled, onChange }) => {
  ...
  const drawTrack = (ctx: CanvasRenderingContext2D, w: number, normalizedOffset: number) => {
    ...
    const borderWidth = 8;
    ctx.beginPath();
    ctx.roundRect(trackX - borderWidth, trackY - borderWidth, TRACK_WIDTH + borderWidth * 2, TRACK_HEIGHT + borderWidth * 2, radius + 2);
    const wallGrad = ctx.createLinearGradient(trackX - borderWidth, trackY - borderWidth, trackX + TRACK_WIDTH + borderWidth, trackY + TRACK_HEIGHT + borderWidth);
    ...
    const innerShadowGrad = ctx.createLinearGradient(trackX, trackY, trackX, trackY + TRACK_HEIGHT);
    ...
    const sideShadow = ctx.createLinearGradient(trackX, trackY, trackX + TRACK_WIDTH, trackY);
    ...
    ctx.beginPath();
    ctx.roundRect(trackX - borderWidth, trackY - borderWidth, TRACK_WIDTH + borderWidth * 2, 2, radius + 2);
  ...
  const animate = (now: number) => {
    ...
    const newOffset = startOffset.current + (targetOffset.current - startOffset.current) * ease;
    const newPos = startPos.current + (targetPos.current - startPos.current) * ease;
  ...
  useEffect(() => {
    ...
    if (canvas) {
      ...
      if (!(window as any).CanvasRenderingContext2D.prototype.roundRect) {
        (window as any).CanvasRenderingContext2D.prototype.roundRect = function(x, y, w, h, r) {
          if (w < 2 * r) r = w / 2;
          if (h < 2 * r) r = h / 2;
          this.moveTo(x+r, y);
          this.lineTo(x+w-r, y);
  ...
  useEffect(() => {
    if (!animating) {
      ...
      if (Math.abs(currentOffset.current - targetOffsetVal) > 0.01 || Math.abs(currentPos.current - targetPosVal) > 0.01) {
  ...
  return (
    <div className={styles.toggleContainer} onClick={() => startToggle(!enabled)}>
      <canvas ref={canvasRef} width={120} height={90} className={styles.canvas} />
    </div>
  );
};